...
class stock_quant(osv.osv):
    ...
    _columns = {
        ...
        'negative_move_id': fields.many2one('stock.move', 'Move Negative Quant', help='If this is a negative quant, this will be the move that caused this negative quant.', readonly=True, select=True),
...
class stock_picking(osv.osv):
    ...
    _columns = {
        ...
        'min_date': fields.function(get_min_max_date, multi="min_max_date", copy=False, fnct_inv=_set_min_date,
        ...
        'max_date': fields.function(get_min_max_date, multi="min_max_date", copy=False,
        ...
        'date': fields.datetime('Creation Date', help="Creation Date, usually the time of the order", copy=False, select=True, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]}, track_visibility='onchange'),
    ...
    def recompute_remaining_qty(self, cr, uid, picking, context=None):
        ...
        def _create_link_for_index(operation_id, index, product_id, qty_to_assign, quant_id=False, context=None):
        ...
        def _create_link_for_quant(operation_id, quant, qty, context):
            """create a link for given operation and reserved move of given quant, for the max quantity possible, and returns this quantity"""
            if not quant.reservation_id.id:
                return _create_link_for_product(operation_id, quant.product_id.id, qty, context=context)
            qty_on_link = 0
            for i in range(0, len(prod2move_ids[quant.product_id.id])):
                if prod2move_ids[quant.product_id.id][i]['move'].id != quant.reservation_id.id:
                    continue
                context['cantidad_move_filtrados'] = 0
                qty_on_link = _create_link_for_index(operation_id, i, quant.product_id.id, qty, quant_id=quant.id, context=context)
                break
            return qty_on_link

        def _create_link_for_product(operation_id, product_id, qty, context):
            ...
            if prod2move_ids.get(product_id):
                context['cantidad_move_filtrados'] = 0
                while prod2move_ids[product_id] and qtyassign_cmp > 0 and len(prod2move_ids[product_id]) > context['cantidad_move_filtrados']:
            ...
            if ops.package_id and not ops.product_id:
                ...
                for quant in quant_obj.browse(cr, uid, quant_ids, context=context):
                    ...
                    if quant.reservation_id:
                        ...
                        qty_on_link = _create_link_for_quant(ops.id, quant, quant.qty, context=context)
                ...
                for move_dict in prod2move_ids.get(ops.product_id.id, []):
                    ...
                    for quant in move.reserved_quant_ids:
                        ...
                        if flag:
                            ...
                            qty_on_link = _create_link_for_quant(ops.id, quant, max_qty_on_link, context=context)
                            qty_to_assign -= qty_on_link
...
class stock_move(osv.osv):
    ...
    _columns = {
        ...
        'partner_id': fields.many2one('res.partner', 'Destination Address ', states={'done': [('readonly', True)]}, select=True, help="Optional address where goods are to be delivered, specifically used for allotment"),
        ...
        'price_unit': fields.float('Unit Price', copy=False, help="Technical field used to record the product cost set by the user during a picking confirmation (when costing method used is 'average price' or 'real'). Value given in company currency and in product uom."),  # as it's a technical field, we intentionally don't provide the digits attribute

        'company_id': fields.many2one('res.company', 'Company', required=True, select=True),
        'split_from': fields.many2one('stock.move', string="Move Split From", help="Technical field used to track the origin of a split move, which can be useful in case of debug", copy=False, select=True),
        ...
        'procurement_id': fields.many2one('procurement.order', 'Procurement', select=True),
        ...
        'picking_type_id': fields.many2one('stock.picking.type', 'Picking Type', select=True),
        'inventory_id': fields.many2one('stock.inventory', 'Inventory', select=True),
        'lot_ids': fields.function(_get_lot_ids, type='many2many', relation='stock.production.lot', string='Lots'),
        'origin_returned_move_id': fields.many2one('stock.move', 'Origin return move', help='move that created the return move', copy=False, select=True),
        'returned_move_ids': fields.one2many('stock.move', 'origin_returned_move_id', 'All returned moves', help='Optional: all returned moves created from this move'),
        'reserved_availability': fields.function(_get_reserved_availability, type='float', digits=dp.get_precision('Product Unit of Measure'), string='Quantity Reserved', readonly=True, help='Quantity that has already been reserved for this move'),
        ...
        'restrict_lot_id': fields.many2one('stock.production.lot', 'Lot', help="Technical field used to depict a restriction on the lot of quants to consider when marking this move as 'done'", select=True),
    ...
    def check_tracking(self, cr, uid, move, lot_id, context=None):
        ...
        if move.picking_id and (move.picking_id.picking_type_id.use_existing_lots or move.picking_id.picking_type_id.use_create_lots) and \
            ...
            if not lot_id:
                raise UserError(_('Debe proporcionar un Número de Lote / Serie para el producto %s') % move.product_id.name_get()[0][1])
    ...
    def action_done(self, cr, uid, ids, context=None):
        ...
        for ops in operations:
            ...
            for record in ops.linked_move_operation_ids:
                ...
                if float_compare(record.qty, 0, precision_rounding=move.product_id.uom_id.rounding) > 0 and not move_qty.get(move.id):
...
class stock_pack_operation(osv.osv):
    ...
    _columns = {
        ...
        'lot_id': fields.many2one('stock.production.lot', 'Lot/Serial Number', select=True),
        ...
        'reservation_id': fields.many2one('stock.move', 'Reservation', select=True),
        'fresh_record': fields.boolean('Pack Operation Nuevo'),
        'lots_visible': fields.function(_compute_lots_visible, string="Lotes Visibles", type='boolean'),
    ...
    def check_tracking(self, cr, uid, ids, context=None):
        ...
        for ops in operations:
            if ops.picking_id and (ops.picking_id.picking_type_id.use_existing_lots or ops.picking_id.picking_type_id.use_create_lots) and \
                ...
                if not ops.lot_id:
                    raise UserError(_('Debe proporcionar un Número de Lote / Serie para el producto %s') % ops.product_id.name_get()[0][1])
                if ops.product_id.tracking == 'serial' and ops.qty_done != 1.0:
                    raise UserError(_('Debe proporcionar un Número de Serie para cada unidad'))
...
class stock_move_operation_link(osv.osv):
    ...
    _columns = {
        ...
        'reserved_quant_id': fields.many2one('stock.quant', 'Reserved Quant', help="Technical field containing the quant that created this link between an operation and a stock move. Used at the stock_move_obj.action_done() time to avoid seeking a matching quant again", select=True),
    }
...
class stock_picking_type(osv.osv):
    ...
    _columns = {
        ...
        'show_entire_packs': fields.boolean('Mover paquetes enteros'),
        'warehouse_id': fields.many2one('stock.warehouse', 'Warehouse', ondelete='cascade'),
        'active': fields.boolean('Active'),
        'use_create_lots': fields.boolean('Crear Lotes nuevos'),
        'use_existing_lots': fields.boolean('Usar Lotes existentes'),
