(function() {
    ...
    bus.local_storage = window.localStorage;
    try {
        var uid = new Date();
        bus.local_storage.setItem(uid, uid);
        bus.local_storage.removeItem(uid);
    } catch (exception) {
        var storage = {};
        ...
        bus.local_storage = new RamStorage();
    }
    ...
    bus.Bus = openerp.Widget.extend({
        ...
        on_notification: function(notification) {
            var self = this;
            if (notification.id > self.last) {
                self.last = notification.id;
            }
    ...
    var CrossTabBus = bus.Bus.extend({
        ...
        on_notification: function(notification){
            if(this.is_master) { // broadcast to other tabs
                ...
                var new_notifications = _.filter([notification], function (notif) {
                ...
                if (new_notifications.length) {
                    ...
                    this._super(new_notifications[0]);
                }
            } else {
                this._super.apply(this, arguments);
            }
        ...
        on_storage: function (e) {
            ...
            if(e.key === 'bus.notification'){
                ...
                _.each(notifs, _.bind(self.on_notification, self));
            }
            // update channels
    ...
    function getItem(key, defaultValue) {
        var val = bus.local_storage.getItem(key);
        return val ? JSON.parse(val) : defaultValue;
    }
    
    function setItem(key, value) {
        bus.local_storage.setItem(key, JSON.stringify(value));
    }
    ...
    var tab_manager = {
        ...
        register_tab: function (is_master_callback, is_no_longer_master, on_heartbeat_callback) {
            ...
            on('unload', function () {
                ...
                if (tab_manager.isMaster) {
                    bus.local_storage.removeItem(tab_manager.masterKey);
                }
            });
    
            if (!bus.local_storage.getItem(tab_manager.masterKey)) {
        ...
        heartbeat: function () {
            ...
            var heartbeatValue = bus.local_storage.getItem(tab_manager.heartbeatKey) || 0;
            var peers = getItem(tab_manager.peersKey, {});
            ...
            if (tab_manager.isMaster) {
                ...
                if (!tab_manager.is_last_heartbeat_mine()) {
                    ...
                    bus.local_storage.setItem(tab_manager.heartbeatKey, current);
