        ...
        _.each(filters.concat({category:'filters', item: 'separator'}), function (filter) {
            if (filter.item.tag === 'filter' && filter.category === current_category) {
                if (filter.item.attrs.name === 'intervalos_fechas') {
                    ...
                    current_group.push(new my.Filter({attrs: {domain: "[('"+filter.item.attrs.field_name+"','<=', context_today().strftime('%Y-12-31')),('"+filter.item.attrs.field_name+"','>=', context_today().strftime('%Y-01-01'))]", help:"Año actual", string:"Año actual", name: 'año_actual'}, children:[], tag: "filter"}, self));
                    current_group.push(new my.Filter({attrs: {domain: "[('"+filter.item.attrs.field_name+"','<', context_today().strftime('%Y-01-01')),('"+filter.item.attrs.field_name+"','>=', (context_today() - relativedelta(years=1)).strftime('%Y-01-01'))]", help:"Año anterior", string:"Año anterior", name: 'año_anterior'}, children:[], tag: "filter"}, self));
                    return;
                }
...
instance.web.search.DateField = instance.web.search.Field.extend(/** @lends instance.web.search.DateField# */{
    ...
    complete: function (needle) {
        ...
        try {
            ...
            v = instance.web.parse_value(needle, {'widget': t});
        } catch (e) {
            return $.when(null);
        }
